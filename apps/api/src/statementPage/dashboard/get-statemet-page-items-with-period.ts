...
@Injectable()
export class GetStatmentsPageItemsWithPeriodService {
  private readonly logger = new Logger(GetStatmentsPageItemsWithPeriodService.name);
  constructor(
  ...
  async execute(business_id: string, page_id: string, period: string) {
    try {
      if (!period) {
        throw new BadRequestErrorException('please specify a period');
      }

      const getPeriodStatement = await this.statementRepo.findOne({
        where: { business_id, id: page_id },
        relations: ['app'],
      });
      if (!getPeriodStatement) {
        throw new BadRequestErrorException('This page does not exist');
      }

      let accounts;
      const getPeriod = getRangePeriod(period);
      accounts = await this.accountRepo.find({ where: { business_id } });
      if (getPeriod.isCheck) {
        accounts = await this.accountRepo.find({
          where: { business_id, createdAt: getPeriod.startDate },
        });
      }
